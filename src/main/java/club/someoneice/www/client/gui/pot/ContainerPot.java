...
public class ContainerPot extends Container {
    ...
    public ContainerPot(InventoryPlayer player, World world, int x, int y, int z, TilePot tile) {
        ...
        for (int h = 0; h < 2; h ++) {
            for (int l = 0; l < 3; l ++) {
                this.addSlotToContainer(new Slot(inventory, l + h * 3, 18 + l * 18, 22 + h * 18));
            }
        }

        this.addSlotToContainer(new Slot(tile, 6, 102, 58));
        this.addSlotToContainer(new SlotOutput(tile, 7, 102, 32));
