...
public class SeedTagUtil {
    ...
    public static void inputTags() {
        CROPS.forEach(      (it) -> Tags.SEED_TAG.put(it.func_149866_i()));
        WATER_CROPS.forEach((it) -> Tags.SEED_TAG.put(it.func_149866_i()));
        HELL_CROPS.forEach( (it) -> Tags.SEED_TAG.put(it.func_149866_i()));
    }
}
