package club.someoneice.www.common.container;

import club.someoneice.www.common.container.slot.SlotOutput;
...
public class ContainerPot extends Container {
  ...
  public boolean canInteractWith(EntityPlayer player) {
    return worldObj.getBlock(pos.chunkPosX, pos.chunkPosY, pos.chunkPosZ) == W3Blocks.pot && player.getDistanceSq((double) this.pos.chunkPosX + 0.5D, (double) this.pos.chunkPosY + 0.5D, (double) this.pos.chunkPosZ + 0.5D) <= 64.0D;
  }
  ...
  public ItemStack transferStackInSlot(EntityPlayer player, int slotNumber) {
    ...
    if (slotNumber < this.tile.getInventory().getSizeInventory()) {
      if (!this.mergeItemStack(itemstack1, this.tile.getInventory().getSizeInventory(), this.inventorySlots.size(), true))
        return null;
