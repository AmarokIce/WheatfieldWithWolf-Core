...
public class TomatoCrop extends CropFactory {
  public TomatoCrop() {
    super("tomato_seed", W3Items.tomato);
  }
  ...
  public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int face, float hitX, float hitY, float hitZ) {
    ...
    rat.add(new ItemStack(world.rand.nextDouble() < 0.25D ? W3Items.rotten_tomato : W3Items.tomato, world.rand.nextInt(3) + 1));
    if (world.rand.nextDouble() < 0.25D) rat.add(new ItemStack(W3Items.cherry_tomato, 2));
