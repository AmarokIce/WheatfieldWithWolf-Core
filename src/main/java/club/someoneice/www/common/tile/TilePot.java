...
public class TilePot extends TileEntity {
  ...
  private void checkTheRecipe() {
    ...
    for (int i = 0; i < 6; i++)
      if (this.inventory.getStackInSlot(i) != null) itemInput[i] = this.inventory.getStackInSlot(i);
    ...
    if (++this.time < 200 || (recipe.bowl != null && !Util.itemStackEquals(this.inventory.getStackInSlot(6), recipe.bowl)))
      return;
    crafting(recipe);
    this.time = 0;
  }
  ...
  private void crafting(RecipePot recipe) {
    ...
    else if (this.inventory.getStackInSlot(7).getItem() == recipe.output.getItem())
      this.inventory.getStackInSlot(7).stackSize++;
    else return;
  ...
  public boolean canBurn() {
    return W3Tags.HOT_SOURCE.has(this.worldObj.getBlock(this.xCoord, this.yCoord - 1, this.zCoord));
  }
